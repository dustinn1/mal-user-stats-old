...
function getAuthenticationLink() {
  ...
  return {
    statusCode: 302,
    headers: {
      Location: `https://myanimelist.net/v1/oauth2/authorize?response_type=code&client_id=${process.env.MAL_CLIENT_ID}&code_challenge=${challenge}&state=${encryptedChallenge}`,
...
export async function handler(event: HandlerEvent, context: HandlerContext) {
  ...
  if (user) {
    try {
      decodedUserJWT = jwt.verify(user, process.env.JWT_TOKEN_SECRET) as userJWT;
    } catch(err) {
      return {
        statusCode: 400,
    ...
    try {
      const userResult = await db.collection('users').findOne({ mal_id: decodedUserJWT.mal_id });
      ...
      } else if (difference > 1 && difference < 720) { // 720 hours == 1 month (30 days)
